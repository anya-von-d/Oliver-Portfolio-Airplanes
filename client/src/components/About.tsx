...
export default function About() {
  return (
    <section id="about" className="relative py-16 px-4 sm:px-6 overflow-hidden" data-testid="section-about">
      <div className="max-w-6xl mx-auto w-full">
        <div className="mb-16 text-center">
          <h2 className="font-mono text-2xl sm:text-3xl md:text-4xl mb-4 text-[#171511]" data-testid="heading-about">
        ...
        <div>
          <div className="mb-12">
            <div className="space-y-6 animate-slide-in-left text-center">
              <p className="text-base sm:text-lg leading-relaxed text-[#171511] max-w-3xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-4 sm:gap-6">
            {focuses.map((focus, index) => (
              <Card
                key={focus.title}
                className="p-6 hover-elevate active-elevate-2 transition-all cursor-default animate-fade-in bg-white/60 border-[#b8b3ae]"
                style={{ animationDelay: `${index * 100}ms` }}
                data-testid={`card-focus-${focus.title.toLowerCase().replace(/\s+/g, '-')}`}
              >
                <h3 className="font-display font-bold text-lg sm:text-xl mb-2 text-[#171511]">{focus.title}</h3>
                <p className="text-sm sm:text-base text-[#171511]/70 leading-relaxed">{focus.description}</p>
              </Card>
            ))}
          </div>
        </div>
